...
class ParallelSepConvBlock(nn.Module):
    def __init__(self, in_channels, out_channels):
        super().__init__()
        self.left_branch = nn.Sequential(
...
class CustomSepCNN(nn.Module):
    def __init__(self, num_classes: int = 1, model: str = "cuslin"):
        super().__init__()
        self.initial = nn.Sequential(
            SeparableConv2d(3, 32, 3, 1),
            nn.BatchNorm2d(32),
            nn.ReLU(inplace=True),
        ...
        self.blocks = nn.Sequential(
            ParallelSepConvBlock(32, 64),
            ParallelSepConvBlock(64, 128),
            ParallelSepConvBlock(128, 256),
            ParallelSepConvBlock(256, 512),
        )
        self.final_conv = nn.Conv2d(512, 256, kernel_size=1)
